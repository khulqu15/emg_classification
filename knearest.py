...
try:
    ...
    roc_auc = auc(fpr, tpr)
    fpr_le, tpr_le, _ = roc_curve(y_test_le, y_pred_best_le)
    roc_auc_le = auc(fpr_le, tpr_le)

    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, label=f'KNN Original (AUC = {roc_auc:.2f})')
    plt.plot(fpr_le, tpr_le, label=f'KNN Linear Envelope (AUC = {roc_auc_le:.2f})')
