...
try:
    ...
    # Linear Envelope
    def linear_envelope(signal, low_pass_freq=2, sampling_rate=10000):
        # Recrifier sinyal (mengambil nilai absolute)
        rectified_signal = np.abs(signal)
        # BUtterworth low-pass filter
        b, a = butter(N=4, Wn=low_pass_freq/(sampling_rate/2), btype='low')
        filtered_signal = filtfilt(b, a, rectified_signal)
        return filtered_signal
    
    # Penerapan Linear Envelope pada data
    X_train_le = np.apply_along_axis(linear_envelope, 0, X_train_scaled)
    X_test_le = np.apply_along_axis(linear_envelope, 0, X_test_scaled)
    
    # Plotting data asli vs data yang telah diproses
    plt.figure(figsize=(15, 5))
    plt.subplot(1, 2, 1)
    plt.plot(X_train_scaled[0, :], label='Original')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(X_train_le[0, :], label='Linear Envelope')
    plt.title('Linear Envelope Processed Data')
    plt.xlabel('Samples')
    plt.ylabel('Amplitude')
    plt.legend()
