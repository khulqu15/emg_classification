...
try:
    ...
    model_le = keras.Sequential([
        keras.layers.Dense(10, activation='relu', input_shape=(X_train_le_scaled.shape[1],)),
        keras.layers.Dense(10, activation='relu'),
        keras.layers.Dense(1, activation='sigmoid')
    ])
    model_le.compile(
    ...
    epochs = range(1, len(loss_values_le) + 1)
    # Plotting: Plotting process
    plt.figure(figsize=(12, 16))
    plt.subplot(1, 2, 1)
    plt.plot(epochs, loss_values_le, 'bo', label='Training loss')
    plt.plot(epochs, val_loss_values_le, 'b', label='Validation loss')
    plt.title('Training & Validation Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    ...
    plt.subplot(1, 2, 2)
    plt.plot(epochs, accuracy_le, 'bo', label='Training accuracy')
    plt.plot(epochs, val_accuracy_le, 'b', label='Validation accuracy')
    plt.title('Training & Validation accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
