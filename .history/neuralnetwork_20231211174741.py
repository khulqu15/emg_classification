...
try:
    ...
    # Menerapkan full-wave rectification dan low-pass filter
    combined_data['Fleksi_rectified'] = combined_data['Fleksi'].apply(full_wave_rectification)
    combined_data['Ekstensi_rectified'] = combined_data['Ekstensi'].apply(full_wave_rectification)

    combined_data['Fleksi_envelope'] = combined_data['Fleksi_rectified'].apply(lambda x: low_pass_filter(x))
    combined_data['Ekstensi_envelope'] = combined_data['Ekstensi_rectified'].apply(lambda x: low_pass_filter(x))

    # Plotting sinyal asli dan linear envelope
    plt.figure(figsize=(12, 8))

    plt.subplot(2, 1, 1)
    plt.plot(combined_data['Fleksi'], label='Original Fleksi')
    plt.plot(combined_data['Fleksi_envelope'], label='Linear Envelope Fleksi')
    plt.title('Fleksi Signal')
    plt.legend()

    plt.subplot(2, 1, 2)
    plt.plot(combined_data['Ekstensi'], label='Original Ekstensi')
    plt.plot(combined_data['Ekstensi_envelope'], label='Linear Envelope Ekstensi')
    plt.title('Ekstensi Signal')
    plt.legend()
